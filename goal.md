...
>   - The discovery-only endpoint (`/api/cron/grade-threshold-discovery`) is merged (PR #18, merge commit `ce2eaec8de41371113aee090fcc84525f7bdaca2`) and deployed to Production (deployment `dpl_2t1Kbt6hJMMxiPgPz38cAWF459pz`), but dormant (`CRON_SECRET`). On this branch, structured runtime logging is implemented to emit sanitized, single-line JSON logs on every authorized discovery execution for operator observability.
>   - Proposed (not activated) operating model: recommended Production schedule `0 7 * * *` (UTC). On the Hobby plan, invocation may occur at any point from 07:00 through 07:59 UTC, deliberately scheduled after published Cambridge morning results releases. Operator polling relies on the sanitized structured log during the one-hour Hobby retention window (weekly off-season inspection around 07:30 UTC, with a follow-up after 08:00 UTC if delayed; two-stage check on official results days and subsequent days until manifest admission).
...
>   - `vercel.json` remains absent; `CRON_SECRET` remains unprovisioned; no schedule or production cron activation was performed; first authorized Production invocation, immediate disabling, and schedule removal remain separately approved operational actions; import and publication remain separately gated.

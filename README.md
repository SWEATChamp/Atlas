...
- v1.1.0 (Dashboard Mobile Compatibility & Update Notifications): Daily Mission cards now respond to their own available width, using a compact row when space permits and a flexible 2-tier layout in narrow dashboard columns, split-screen windows, tablets, and phones. Mobile overflow fixes, 44×44px touch targets, visible semantic versioning (`Atlas v1.1.0`), and an accessible latest-only "What's New" release update dialog are locally prepared and verified (112/112 unit tests pass); deployment pending.

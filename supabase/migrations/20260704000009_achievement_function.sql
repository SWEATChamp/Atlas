-- ============================================================
-- MIGRATION 009: Achievement Checking Function
-- Separated from 008_functions.sql because it depends on
-- both achievement_definitions table (003) and other tables.
-- check_and_unlock_achievements() is called inside complete_mission()
-- and also directly after paper logging or notes completion.
-- ============================================================
  ...
  LOOP
    ...
    CASE v_rec.key
      ...
      WHEN 'first_blood' THEN
        ...
        FROM public.user_chapters WHERE user_id = p_user_id AND notes_status = 'complete';

      WHEN 'paper_hunter' THEN
        SELECT COUNT(*) > 0 INTO v_condition_met
        FROM public.past_papers WHERE user_id = p_user_id;
      ...
      WHEN 'ace' THEN
        SELECT COUNT(*) > 0 INTO v_condition_met
        FROM public.past_papers WHERE user_id = p_user_id AND accuracy_pct >= 90;

      WHEN 'perfect_score' THEN
        SELECT COUNT(*) > 0 INTO v_condition_met
        FROM public.past_papers WHERE user_id = p_user_id AND accuracy_pct = 100;
    ...
    IF v_condition_met THEN
      ...
      GET DIAGNOSTICS v_rec.sort_order = ROW_COUNT; -- reuse var to check if inserted
      IF v_rec.xp_reward > 0 THEN
        PERFORM public.award_xp(
          p_user_id, v_rec.xp_reward, 'achievement_unlock',
          NULL, jsonb_build_object('achievement_key', v_rec.key)
        );
      END IF;
...
COMMENT ON FUNCTION public.check_and_unlock_achievements IS
  'Evaluates all active, un-earned achievements for a user. '
  'Grants badge + XP atomically. ON CONFLICT prevents duplicates under concurrency.';
